...
export default function OrderWaiting() { 
    ...
    const fetchOrders =  async (userInfo) => {
        ...
        try{
          ...
          if(response.status == 200)
          {
            if(data.length > 0){
                let orderList = [];
                let orderWaitingList = [];
                data.forEach(async(item) => {
                    if(item.status == "Chưa xác nhận")
                    {
                        orderWaitingList.push(item);
                    }
                }) 
                orderWaitingList.forEach(async(item, index) => {
                    if(item.status == "Chưa xác nhận")
                    {
                        ...
                        if(response.status == 200){
                            ...
                            if(index == orderWaitingList.length - 1){
                                setOrders(orderList); 
                            }
                        }
                    }
    ...
    return (
        <PusrchaseItem 
            orders = {orders}
            loading = {loading}
            onForceupdateMethod = {handleForceupdateMethod}
        />
    );
  }  